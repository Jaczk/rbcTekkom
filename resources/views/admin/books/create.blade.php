                ...
                <form enctype="multipart/form-data" method="POST" action="{{ route('admin.book.store') }}">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="custom-select" name="spec_id">
                            @foreach ($specialization as $special)
                                <option value="{{ $special->id }}" {{ old('spec_id') == $special->id ?
                                'selected' : '' }}>
                                    {{ $special->desc }}</option>
                            @endforeach
                        </select>
                        ...
                        <select class="custom-select" name="spec_detail_code">
                            @foreach ($specDetails as $specDetail)
                                <option value="{{ $specDetail->id }}"
                                    {{ old('spec_detail_code') == $specDetail->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="custom-select" name="condition">
                            ...
                            <option value="new" @selected(old('condition') == 'new')
                            @class(['bg-warning text-white' => old('condition') == 'new'])>BARU
                            </option>
                            <option value="used" @selected(old('condition') == 'used')
                            @class(['bg-warning text-white' => old('condition') == 'used'])>NORMAL
                            </option>
                        </select>
                    </div>
                    <div class="form-group">
